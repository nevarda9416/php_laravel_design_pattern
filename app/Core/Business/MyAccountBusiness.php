...
class MyAccountBusiness
{
    ...
    {
        $result = array('responseCode' => '02', 'message' => 'Không lấy được thông tin của người dùng', 'data' => array('customerId' => '', 'phone' => '', 'email' => ''
        , 'fullName' => '', 'dateOfBirth' => '', 'gender' => '', 'isPhoneVerified' => '', 'isEmailVerified' => '', 'isGoogleLogin' => '', 'isFacebookLogin' => ''
        , 'primaryRegister' => '', 'hasPassword' => 0, 'shippingAddress' => ''));
        if (isset($_COOKIE[CommonEnum::COOKIE_USER_TOKEN])) {
            ...
            if ($checkTokenUser == true) {
                ...
                if (!empty($response['responseCode'])) {
                    $result = array('responseCode' => $response['responseCode'], 'message' => $response['message'], 'data' => $response['data']);
                }
            }
        }
    ...
    public static function addFavoritePost($data)
    {
        try {
            ...
            $result = array('responseCode' => $dataM['responseCode'], 'data' => $dataM['data']);
        } catch (\Exception  $e) {
            $result = array('responseCode' => '02', 'data' => array());
    ...
    function getMyOrders($request, $customerId, $slug) {
     ...
     * @throws \Throwable
     */
    function getMyOrdersV3($request, $customerId, $slug) {
        $curPage = (int)$request->get('page') ? (int)$request->get('page') : 1;
        $page = $curPage - 1;
        $take = CommonEnum::LIMIT_DATA_PAGINATE;
        $start = $page * $take;
    ...
    function getMyMedical($request, $customerId, $slug) {
        $curPage = (int)$request->get('page') ? (int)$request->get('page') : 1;
        $page = $curPage - 1;
        $take = CommonEnum::LIMIT_DATA_PAGINATE;
        $start = $page * $take;
     ...
     * @throws \Throwable
     */
    function myFavoritePosts($request, $customerId, $slug)
    {
        $curPage = (int)$request->get('page') ? (int)$request->get('page') : 1;
        ...
        if (config()->get('constants.MY_ACCOUNT_VERSION') == 'v3' && $agent->isMobile() && !$agent->isTablet())
            return view('v3.myaccount.news')->with([
                'paginatorUtility' => $paginatorUtility
                , 'lst_posts' => $lst_posts
            ])->render();
        else
    ...
    function getMyQuestions($request, $customerId, $slug) {
        $curPage = (int)$request->get('page') ? (int)$request->get('page') : 1;
        $page = $curPage - 1;
        $take = CommonEnum::LIMIT_DATA_PAGINATE;
        $start = $page * $take;
    ...
    function getMyAddress($request, $customerId, $slug) {
        $curPage = (int)$request->get('page') ? (int)$request->get('page') : 1;
        $page = $curPage - 1;
        $take = CommonEnum::LIMIT_DATA_PAGINATE;
        $start = $page * $take;
    ...
    function getMyPointsV3($request, $customerId, $slug) {
        $curPage = (int)$request->get('page') ? (int)$request->get('page') : 1;
        $page = $curPage - 1;
        $take = CommonEnum::LIMIT_DATA_PAGINATE;
        $start = $page * $take;
    ...
    function getMyPoints($request, $customerId, $slug) {
        $curPage = (int)$request->get('page') ? (int)$request->get('page') : 1;
        $page = $curPage - 1;
        $take = CommonEnum::LIMIT_DATA_PAGINATE;
        $start = $page * $take;
