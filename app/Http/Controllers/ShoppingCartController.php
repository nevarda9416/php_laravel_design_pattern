...
class ShoppingCartController extends Controller
{
    ...
    public function listProducts()
    {
        ...
        return response()->json(array('success' => true, 'totalItem' => $totalItem, 'content' => $content));
    }
        ...
        try {
            ...
            if (!empty($cart_data)) {
                ...
                if (!empty((array)$setting))
                    ...
                    $setting = array('telephone_contact' => '');
                ...
                if ($cart_data->payment_method == 'COD') {
                    ...
                    if (!empty($inputData['vnp_SecureHash'])) {
                        ...
                        if ($secureHash == $vnp_SecureHash) {
                            ...
                            } else if (isset($inputData['vnp_ResponseCode']) && $inputData['vnp_ResponseCode'] == '00' && isset($inputData['vnp_TransactionNo']) && $inputData['vnp_TransactionNo'] != '0') {
            ...
            try {
                if ($secureHash == $vnp_SecureHash) {
                    ...
                    if ($orderLog) {
                        if (isset($inputData['vnp_Amount']) && $inputData['vnp_Amount'] != ($orderLog->order_value)) {
                        ...
                        } else if ((int)$orderLog->status != 0) {
                            $returnData['RspCode'] = '02';
                            $returnData['Message'] = 'Order already confirmed';
                        } else if (isset($inputData['vnp_ResponseCode']) && $inputData['vnp_ResponseCode'] == '00'
                            ...
                            foreach ($request->all() as $key => $value) {
                                $query .= urlencode($key) . "=" . urlencode($value);
                                if ($key < count($request->all()) - 1) $query .= '&';
                            }
    ...
    public function applyShipping(Request $request)
    {
        ...
        if ($cart_data) {
            ...
            $promotionHtml = view('cart.list_promotion')->with(['promotions' => $promotions, 'promotionL' => $promotionL, 'discounts' => $discounts])->render();
            return json_encode(array('status' => 200, 'html' => $html, 'promotionHtml' => $promotionHtml));
        } else {
            return json_encode(array('status' => 404, 'message' => 'Giỏ hàng không tồn tại'));
        }
        ...
        if ($s['responseCode'] == '00') {
            ...
            if ($s0['responseCode'] == '00') { // Số điện thoại đã tồn tại
                ...
                switch ($s0['data']['primaryRegister']) {
                    case 'Facebook':
                        $re = json_encode(array('error' => 0, 'exist' => 1, 'message' => 'Số điện thoại đã được chuyển từ tài khoản Facebook ' . $s0['data']['fullName'] . ' sang tài khoản hiện tại.'));
                        break;
                    case 'Google':
                    case 'Email':
                        $re = json_encode(array('error' => 0, 'exist' => 1, 'message' => 'Số điện thoại đã được chuyển từ tài khoản ' . $s0['data']['primaryRegister'] . ' ' . $s0['data']['email'] . ' sang tài khoản hiện tại.'));
                        break;
                    default: // Source chính là Phone
                        $re = json_encode(array('error' => 0, 'exist' => 1, 'message' => 'Số điện thoại đã được chuyển từ tài khoản ' . $s0['data']['fullName'] . ' sang tài khoản hiện tại.'));
                        break;
                }
            } else { // Số điện thoại chưa tồn tại
                $d0 = ['customerId' => (int)$customerId, 'phone' => $telephone];
                MyAccountBusiness::update_profile_save($d0);
                $re = json_encode(array('error' => 0, 'exist' => 0, 'message' => 'Đã gán số điện thoại này vào account đang đăng nhập hiện tại'));
            }
            ...
            $re = json_encode(array('error' => 1, 'message' => 'Mã xác thực không chính xác'));
        }
        return $re;
    }
    ...
    public function getOrderContact()
    {
        if (isset($_COOKIE[CommonEnum::COOKIE_USER_DATA])) {
            ...
            $user_data = array('phone' => '', 'email' => '', 'fullName' => '', 'dateOfBirth' => '', 'gender' => '');
        }
    ...
    public function createOrderLDP(Request $request)
    {
        ...
        if ($request->get('tags')) {
            ...
            foreach ($request->get('tags') as $tag) {
                array_push($listTags, array('tagId' => (int)$tag));
            }
            $data['tags'] = $listTags;
        }
        ...
        try {
            ...
            $result = json_encode(array('error' => 0, 'message' => $messageSuccess, 'data' => $data));
            return $result;
        } catch (Exception $exception) {
            Log::error($exception->getMessage(), array('error' => 1, 'message' => $messageError, 'data' => $data));
            return json_encode(array('error' => 0, 'message' => $messageSuccess, 'data' => $data));
        }
    }
}
